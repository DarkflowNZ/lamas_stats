		...
		elseif c == C_LT then
			...
			if c1 == C_BANG then
				if str_index(data, i + 2) == C_DASH and str_index(data, i + 3) == C_DASH then
					-- <!-- comment -->
					i = i + 4
					while i < len and not (str_index(data, i) == C_DASH and str_index(data, i + 1) == C_DASH and str_index(data, i + 2) == C_GT) do
						i = i + 1
					end
					i = i + 3
				else
...
local function tokenizer_read_unquoted_string(self)
	...
	local i = self.cur_idx
	while i < len do
		if delim[str_index(data, i)] then break end
		i = i + 1
	end
	self.cur_idx = i
	return str_sub(data, start_idx, i - start_idx)
...
local function to_string_internal(elem, packed, indent_char, cur_indent)
	local buffer = {}
	-- depth-indexed indent cache, seeded with the caller's starting indent
